...
bool ConfigFile::SetSource(const char *file, bool ignorecase)
{
	...
	if(file != 0)
	{
		...
		if(!f)
		{
			sLog.Error("Could not open %s.", file);
			return false;
		}
		...
		std::map<uint32, ConfigSetting> current_block_map;
...
ConfigSetting * ConfigFile::GetSetting(const char * Block, const char * Setting)
{
	...
	std::map<uint32, std::map<uint32, ConfigSetting> >::iterator itr = m_settings.find(block_hash);
	if(itr != m_settings.end())
	{
		std::map<uint32, ConfigSetting>::iterator it2 = itr->second.find(setting_hash);
